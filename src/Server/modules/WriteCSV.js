...
function UpdateWorldDataFile(updatedWorldData, dataType) {
	...
	function recordValues(item) {
		var dateValueDict;
		if (dataType == DataType.CONFIRMED) dateValueDict = item.numConfirmed;
	...
	var fileName = '';
	if (dataType == DataType.CONFIRMED) fileName = WorldConfirmedFileName;
	else if (dataType == DataType.DEAD) fileName = WorldDeathsFileName;
	else if (dataType == DataType.RECOVERED) fileName = WorldRecoveredFileName;
...
function UpdateUSDataFile(updatedUSData, dataType) {
	...
	function recordValues(item) {
		...
		if (dataType == DataType.DEAD) {
			values += Cell(item.population);
		}
	...
	if (dataType == DataType.CONFIRMED) fileName = USConfirmedFileName;
	else if (dataType == DataType.DEAD) fileName = USDeathsFileName;
...
function RecordCases(updatedAllCases) {
	...
	writeFileSync(CasesFileName, contents);
}
