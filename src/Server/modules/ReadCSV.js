...
const {
	USConfirmedFileName,
	USDeathsFileName,
	WorldConfirmedFileName,
	WorldDeathsFileName,
	WorldRecoveredFileName,
	CasesFileName,
} = require('../modules/DatabaseFileNames.js');
const { Case } = require('../modules/DataClasses.js');
const { USPlace, WorldPlace } = require('../modules/DataClasses.js');
...
function LoadUSData() {
	...
	// Load death data
	let parsedDeathData = ReadCSV(USDeathsFileName);
	...
	deathDateLabels.forEach((date) => deathDates.push(stringToDate(date)));
	...
	for (var i = 1; i < parsedDeathData.length; ++i) {
		...
		var newState = new USPlace(state, county);
	...
	let parsedConfirmedData = ReadCSV(USConfirmedFileName);
	...
	confirmedDatesDateLabels.forEach((date) =>
		confirmedDates.push(stringToDate(date))
	);
	...
	for (var i = 1; i < parsedConfirmedData.length; ++i) {
		...
		if (!(county + ',' + state in statesDict)) {
			statesDict[county + ',' + state] = new USPlace(state, county);
...
function LoadWorldData() {
	...
	// Load death data
	let parsedDeathData = ReadCSV(WorldDeathsFileName);
	...
	for (var i = 1; i < parsedDeathData.length; ++i) {
		...
		var newState = new WorldPlace(country, state, latitude, longitude);
		newState.addNumDeaths(
			deathDates,
			parsedDeathData[i].slice(numNonDateColumn)
		);
	...
	// Organize confirmed data.
	let parsedConfirmedData = ReadCSV(WorldConfirmedFileName);
	...
	for (var i = 1; i < parsedConfirmedData.length; ++i) {
		...
		if (!(state + ',' + country in statesDict)) {
			...
			statesDict[state + ',' + country] = new WorldPlace(
				country,
				state,
				latitude,
				longitude
